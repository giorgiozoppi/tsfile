...
class GenericDataPoint {
   ...
   public:
    GenericDataPoint(const ts_datatype& type, const std::string& measurement_id) : _type(type), _measurement_id(measurement_id) {}
...
template <DataPoint DataPointImpl> std::unique_ptr<DataPointImpl> MakeDataPoint(ts_datatype& type, std::string measurement_id) {
    switch(type) {
